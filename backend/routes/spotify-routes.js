...
// Express Router for spotify routes
...
/**
 * Route to get user's Spotify token
 */
...
/**
 * Route to perform Spotify logout, we update 
 *  the token to be empty for a user
 */
spotifyRoutes.get("/spotify-logout", async (req, res) => {
    try {
        const loggedInUserId = req.cookies.loggedInUser;

        // Checks if we are properly logged in to website and updates our token to be empty
        if (loggedInUserId) {
...
// Export the spotifyRoutes for use in other files
