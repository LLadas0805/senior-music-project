...
// Function to format track duration from milliseconds to minutes:seconds format
...
// Component to display a custom tracklist
        ...
        <div>
          ...
          {discNumbers.map((discNumber) => (
            <div key={`disc-${discNumber}`}>
              ...
              {/* Render tracks based on the alternate mode */}
              {alternate === true ? (
                discs[discNumber].map((item, index) => (
                  <Col key={item.id} xs={12}>
                    ...
                    <div className="tracklist-item">
                      ...
                      <div className="name-artist">
                        {/* Display song name and artists with links */}
                      ...
                      <span className="song-length">{formatDuration(item.duration_ms)}</span>
                    </div>
                  </Col>
                ))
              ) : (
                discs[discNumber].map((item, index) => (
                  <Col key={item.id} xs={12}>
                    ...
                    <div className="tracklist-item">
                      ...
                      <div className="name-artist">
                        {/* Display song name, artists with links, album name */}
                        <span className="song-name">{item.name}</span>
                        <span className="artist-name">
                          {item.artists.map((artist, index) => (
                            <React.Fragment key={index}>
                      ...
                      {/* Display album name and song duration */}
